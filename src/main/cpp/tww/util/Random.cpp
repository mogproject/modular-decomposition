...
static void sampleint_pool(tww::util::Random &rand, Int n, Int k, std::vector<Int> &ret, Int offset = 0) {
  std::vector<Int> xs;
 ...
 */
template <typename Int>
static void vitter_method_a(tww::util::Random &rand, Int n, Int k, std::vector<Int> &ret, Int offset = 0) {
  Int cursor = offset;
  n -= offset;
  assert(k <= n);
  if (k == 0) return;
...
template <typename Int>
static double vitter_get_x(tww::util::Random &rand, Int n, Int k, double p) {
  while (true) {
    double x = n * (1 - p);
    if (x < n - k + 1) return x;
...
template <typename Int>
static Int vitter_get_skip_distance(tww::util::Random &rand, Int n, Int k, double &p) {
  while (true) {
 ...
 */
template <typename Int>
static std::vector<Int> vitter_method_d(tww::util::Random &rand, Int n, Int k) {
  assert(0 <= k && k <= n);
