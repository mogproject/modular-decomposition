...
class IntRootedForest {
 ...
 public:
  ...
  void replace(int index, int replace_by) {
    ...
    if (tww::util::contains(get_ancestors(index), replace_by))
  ...
  void move_to_before(int index, int target) {
    ...
    if (tww::util::contains(get_ancestors(target), index))
  ...
  void add_children_from(int index, int target) {
    ...
    if (tww::util::contains(get_ancestors(index), target))
   ...
   * This original node will be detached form its tree.
   */
  ...
  void check_consistency() const {
    ...
    for (int i = 0; i < static_cast<int>(capacity()); ++i) {
      ...
      if (nodes_[i].parent != NOT_AVAILABLE) {
        ...
        if (!tww::util::contains(cs, i)) throw std::runtime_error("parent must have this as a child");
      }
    }
